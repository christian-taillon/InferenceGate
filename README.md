...
**Version compatibility:** tested against `litellm==1.82.0` (the version this repo pins). The package declares `litellm>=1.82.0`; guardrail hook dispatch is litellm-internal behavior, so after changing LiteLLM versions run the credential-free compatibility suite from this repo: `uv run pytest tests/ -m integration`.
